...
public:
	...
	LWRGazeboComponent(std::string const& name) :
			RTT::TaskContext(name), steps_rtt_(0), steps_gz_(0), n_joints_(0), rtt_done(
...
//		this->addProperty("use_ros_topics", using_ros_topics);
	...
	virtual bool gazeboConfigureHook(gazebo::physics::ModelPtr model) {
		...
		RTT::log(RTT::Error) << "Model has " << gazebo_joints_.size()
				<< " joints" << RTT::endlog();
		RTT::log(RTT::Error) << "Model has " << model_links_.size() << " links"
				<< RTT::endlog();
		// Get the joint names
		...
		for (gazebo::physics::Link_V::iterator it = model_links_.begin();
			...
			RTT::log(RTT::Error) << "Link " << (*it)->GetName()
					<< RTT::endlog();
			if ((*it)->IsStatic()) {
			...
			for (gazebo::physics::Joint_V::iterator jit = joints.begin();
				...
				RTT::log(RTT::Error) << "Adding joint " << name
						<< RTT::endlog();
			}
		...
		RTT::log(RTT::Error) << "Model has " << nb_static_joints
				<< " static joints" << RTT::endlog();
		...
		RTT::log(RTT::Error) << "Gazebo model found " << n_joints_ << " joints "
				<< RTT::endlog();
		...
		RTT::log(RTT::Error) << "Done configuring gazebo" << RTT::endlog();
		last_update_time_ = RTT::os::TimeService::Instance()->getNSecs(); //rtt_rosclock::rtt_now();
	...
	virtual void gazeboUpdateHook(gazebo::physics::ModelPtr model) {
		...
		do {
			...
			if (rtt_time_ != rtt_last_clock && data_fs != RTT::NewData)
//				RTT::log(RTT::Debug) << getName() << " "
//						<< "Waiting for UpdateHook at "
//						<< rtt_rosclock::host_now() << " v:" << nb_cmd_received_
//						<< data_fs << RTT::endlog();
				rtt_last_clock = rtt_time_;
		...
		for (unsigned j = 0; j < n_joints_; j++) {
			jnt_pos_[j] =
					gazebo_joints_[j + nb_static_joints]->GetAngle(0).Radian();
...
//		jnt_trq_cmd_.push_back(0.8);
		...
		case RTT::OldData:
			break;
		case RTT::NewData:
			RTT::log(RTT::Debug) << getName() << " " << data_fs << " at "
...
////					&& js_cmd.header.frame_id == "POSITION_CMD") {
...
//	bool using_ros_topics;
	...
	RTT::nsecs last_data_timestamp, data_timestamp, last_timestamp;
